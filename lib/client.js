...
class Client extends EventEmitter {
  constructor(options) {
    ...
    /** @type {Map<string,{timeout:NodeJS.Timeout, cb:Function}>} */
  ...
  // Helper utils
  // build a stable key from address (+net,adr) and invokeId -----
  _makeInvokeKey(address, invokeId) {
    let ip = '', net = '', adr = '';
    if (address) {
      if (typeof address === 'string') {
        ...
        ip  = address.ip  ?? '';
        net = address.net ?? '';
        if (Array.isArray(address.adr)) {
          adr = address.adr.join('.');
        } else if (Buffer.isBuffer(address.adr)) {
          adr = Array.from(address.adr).join('.');
        } else if (address.adr != null) {
          adr = String(address.adr);
        }
      }
    }
    return `${ip}|${net}|${adr}|${invokeId}`;
  }
  // ------------------------------------------------------------------------
  ...
  _invokeCallback(address, invokeId, err, result) {
    const key = this._makeInvokeKey(address, invokeId);
    const entry = this._invokeStore.get(key);
    if (!entry) return debug(`Invoke ${key} received - callback ${!!entry ? 'found' : 'not found' }`);
    clearTimeout(entry.timeout);
    this._invokeStore.delete(key);
    debug(`Invoke ${key} received - callback removed`);
    entry.cb(err, result);
   }

  _addCallback(address, invokeId, cb) {
    const key = this._makeInvokeKey(address, invokeId);
    const timeout = setTimeout(() => {
      this._invokeStore.delete(key);
      cb(new Error('ERR_TIMEOUT'));
    }, this._settings.apduTimeout);
    this._invokeStore.set(key, {timeout, cb});
    debug(`Invoke ${key} added - timeout: ${this._settings.apduTimeout}ms`);
  }
  ...
  // Service Handlers
  _processError(address, invokeId, buffer, offset, length) {
    const result = baServices.error.decode(buffer, offset, length);
    if (!result) return debug('Couldn`t decode Error');
    this._invokeCallback(address, invokeId, new Error('BacnetError - Class:' + result.class + ' - Code:' + result.code));
  }
  ...
  _handlePdu(address, type, buffer, offset, length, addressObject) {
    let result;
    ...
    switch (type & baEnum.PDU_TYPE_MASK) {
      ...
      case baEnum.PduTypes.SIMPLE_ACK:
        ...
        offset += result.len;
        length -= result.len;
        this._invokeCallback(addressObject, result.invokeId, null, {result: result, buffer: buffer, offset: offset + result.len, length: length - result.len});
        break;
      ...
      case baEnum.PduTypes.ERROR:
        ...
        this._processError(address, result.invokeId, buffer, offset + result.len, length - result.len);
        break;
      case baEnum.PduTypes.REJECT:
      case baEnum.PduTypes.ABORT:
        result = baApdu.decodeAbort(buffer, offset);
        this._processAbort(addressObject, result.invokeId, result.reason);
        break;
      case baEnum.PduTypes.CONFIRMED_REQUEST:
        result = baApdu.decodeConfirmedServiceRequest(buffer, offset);
        if ((type & baEnum.PduConReqBits.SEGMENTED_MESSAGE) === 0) {
          this._processConfirmedServiceRequest(addressObject, result.type, result.service, result.maxSegments, result.maxApdu, result.invokeId, buffer, offset + result.len, length - result.len);
        } else {
  ...
  _handleNpdu(buffer, offset, msgLength, remoteAddress) {
    ...
    this._handlePdu(remoteAddress, apduType, buffer, offset, msgLength,addressObject);
  }
  ...
  readProperty(address, objectId, propertyId, options, next) {
	...
	this._transport.send(buffer.buffer, buffer.offset, sendAddress);
    this._addCallback(address, settings.invokeId, (err, data) => {
